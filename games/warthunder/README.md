...
Native Linux build, no Proton prefix.
    ...
    --game-dir DIR   the install, instead of the lookup

Only the `controls{}` block of `machine.blk` is rewritten; every other block
...
Axes sit in `controls{ axes{ } }`:
    ...
    innerDeadzone  nonlinearity  kMul  kAdd  relative  relSens  inverse

The plan owns whatever it names in `AXIS_DEADZONE` and `AXIS_PROPS` and
preserves the rest, so the game's own calibration survives.
...
Numbering is global across devices, out of `deviceMapping` in `machine.blk`:
...
pad" entries that shift everything after them. The game renumbers its existing
bindings when this happens; the tool reads the offsets live rather than
assuming them.
...
form applies to aircraft *and* helicopters. `contexts()` is the only place
that knows this; a clash it reports stops the run.
...
`ID_TRIM_ELEVATOR_PLUS/MINUS` and friends, in a different section of the
controls menu.

**Rudder deadzone is widened to 0.10** in `AXIS_DEADZONE`; `helicopter_pedals`
is the same physical axis and stays at 0.02.
...
**The per-axis multipliers sit outside `controls{}`.** `rudderMultiplier` and
its kind are a slider in the game's own UI, and survive regeneration.

**Not every aircraft has trim.** The game has its own messages for it, so a
trim binding doing nothing may be the airframe:

    No elevator trim control
    Trim unavailable: elevator inoperative
